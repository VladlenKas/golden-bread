...
import { Form, FormFieldInput, FormField } from '@/shared/ui/form';
...
const { handleRegister, isDownloadingAgreement, downloadPreviewAgreement } = useRegister();
...
function areMainFieldsValid(values: any): boolean {
  if (!values) return false;

  const requiredFields = ['name', 'inn', 'ogrn', 'email', 'password', 'passwordConfirm'];
  ...
  if (values.inn?.length !== 10) return false;
  if (values.ogrn?.length !== 13) return false;
...
<template>
  <div
    class="min-h-screen w-full bg-gradient-to-br from-background via-muted/30 to-background flex items-center justify-center p-4 sm:p-6 lg:p-8 relative overflow-hidden -my-6">
    <!-- Декоративные круги -->
    ...
    <div class="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center relative z-10">
      <div class="hidden lg:flex flex-col justify-center p-8 relative">
        <div class="max-w-lg space-y-6">
          <div class="space-y-2">
            ...
            <p class="text-muted-foreground text-lg">Личный кабинет для B2B-клиентов с историей заказов, аналитикой и
              быстрым повтором заказа.</p>
          </div>

          <!-- Имитация интерфейса (мокап) -->
          <div class="relative mt-8">
            <div
              class="absolute -inset-4 bg-gradient-to-r from-primary/20 to-accent/20 rounded-3xl blur-2xl opacity-50" />
            <div
              class="relative bg-card rounded-2xl border border-border shadow-2xl p-4 space-y-3 rotate-[-2deg] hover:rotate-0 transition-transform duration-500">
              <!-- Шапка мокапа -->
      ...
      <div class="flex justify-center lg:justify-end perspective-1000">
        <Transition mode="out-in" enter-active-class="transition-all duration-300 ease-out"
          enter-from-class="opacity-0 translate-x-8 rotate-y-12" enter-to-class="opacity-100 translate-x-0 rotate-y-0"
          ...
          <Card v-if="isLogin" key="login" class="w-full max-w-md bg-card/95 backdrop-blur-xl border-border shadow-2xl">
            ...
            <CardContent class="p-6 pt-0">
              <Form v-slot="{ handleSubmit, isSubmitting }" as="" keep-values :validation-schema="loginSchema">
                <form class="space-y-4" @submit="handleSubmit($event, values => handleLogin(values as LoginRequest))">
                  ...
                  <Button type="submit" :disabled="isSubmitting" class="w-full gap-2 mt-2">
          ...
          <Card v-else key="register" class="w-full max-w-md bg-card/95 backdrop-blur-xl border-border shadow-2xl">
            <CardHeader class="space-y-1 p-6">
              <div class="flex items-center gap-2 mb-2">
                <button @click="isLogin = true" class="text-muted-foreground hover:text-foreground transition-colors">
            ...
            <CardContent class="p-6 pt-0">
              <Form v-slot="{ handleSubmit, isSubmitting, setErrors, meta, values }" as="" keep-values
                :validation-schema="registerSchema" :initial-values="{ agreedToTerms: false }">
                <form class="space-y-4"
                  ...
                  <FormFieldInput name="password" label="Пароль" type="password" placeholder="••••••••" />
                  <FormFieldInput name="passwordConfirm" label="Подтвердите пароль" type="password"
                    placeholder="••••••••" />

                  <!-- ЧЕКБОКС -->
                  <FormField v-slot="{ componentField }" name="agreedToTerms">
                    <FormItem class="flex flex-row items-start space-x-3 space-y-0">
                      <FormControl>
                        <Checkbox :checked="componentField.modelValue" @update:checked="componentField.onChange"
                          :disabled="!areMainFieldsValid(values)" />
                      </FormControl>
                      <div class="space-y-1 leading-none">
                        <FormLabel class="text-sm text-muted-foreground font-normal cursor-pointer">
                          Я ознакомлен(а) и согласен(на) с
                          <button type="button"
                            class="text-primary hover:underline font-medium inline-flex items-center gap-1 disabled:opacity-50"
                            :disabled="!areMainFieldsValid(values) || isDownloadingAgreement"
                            @click="downloadPreviewAgreement(values)">
                            <span v-if="isDownloadingAgreement">
                              <Loader2 class="w-3 h-3 animate-spin inline" />
                              Формирование...
                            </span>
                            <span v-else>условиями сотрудничества</span>
                          </button>
                        </FormLabel>
                        <FormMessage />
                      </div>
                  ...
                  <Button type="submit" :disabled="isSubmitting || !values.agreedToTerms" class="w-full gap-2 mt-2">
                    <Loader2 v-if="isSubmitting" class="w-4 h-4 animate-spin" />
