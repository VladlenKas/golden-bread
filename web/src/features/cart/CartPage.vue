    ...
    <div v-if="items.length > 0" class="grid grid-cols-1 lg:grid-cols-[1fr_330px] gap-6">
      ...
      <div class="space-y-4">
        ...
        <TransitionGroup name="list" tag="div" class="space-y-4">
          <CartItemCard v-for="item in items" :key="item.productBatchId" :item="item"
            @toggle-selection="toggleSelection" @increment="incrementQuantity" @decrement="decrementQuantity"
            @remove="removeItem" @toggle-favorite="toggleFavorite" />
        </TransitionGroup>
      </div>
              ...
              <Popover>
                <PopoverTrigger as-child>
                  <Button variant="outline" class="w-full justify-between text-left font-normal"
                    :class="{ 'text-muted-foreground': !selectedDate }" :disabled="isCalendarDisabled">
                    {{ formattedDate }}
                    <CalendarIcon class="h-4 w-4 text-muted-foreground opacity-60" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent class="w-auto p-0" align="start">
                  <Calendar v-model="selectedDate as DateValue" mode="single" :min-value="minDate" :max-value="maxDate"
                    :disabled="isCalendarDisabled" initial-focus />
                </PopoverContent>
              </Popover>
...
<div class="space-y-3">
  ...
  <div class="space-y-1.5">
    <div class="flex items-center justify-between text-sm">
      <span class="flex items-center gap-2" :class="isWithinProductionLimit ? 'text-foreground' : 'text-destructive'">
        <Clock class="w-4 h-4" />
        Время производства
      </span>
      <span class="font-semibold tabular-nums" :class="isWithinProductionLimit ? 'text-foreground' : 'text-destructive'">
        {{ formattedProductionTime }}
      </span>
    </div>
    ...
    <div class="h-1 bg-muted rounded-full overflow-hidden">
      <div class="h-full transition-all duration-500 ease-out"
        :class="isWithinProductionLimit ? 'bg-primary' : 'bg-destructive'"
        :style="{ width: productionProgressPercent + '%' }">
      </div>
    </div>
    
    <p v-if="!isWithinProductionLimit" class="text-xs text-destructive flex items-center gap-1">
      Превышен лимит 8 часов
    </p>
  </div>
</div>
            ...
            <!-- Кнопка оформления (обновлённая) -->
            <Button class="w-full gap-2 h-12 text-base font-semibold shadow-md hover:shadow-lg transition-shadow"
              size="lg" @click="selectedDate && submitOrder(selectedDate as CalendarDate)"
              :disabled="summary.selectedItems === 0 || !selectedDate || isCreatingOrder || !isWithinProductionLimit">
